...
interface WordDao {
    ...
    @Query("SELECT w504.id, w504.word, w504.code, w504.pronunciation, w504.sample, w504.definition, w504.translate FROM w504, state WHERE w504.id = state.id AND w504.id IN (SELECT state.id FROM state WHERE review IS NOT null AND number == :box)")
